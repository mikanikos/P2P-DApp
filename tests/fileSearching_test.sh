...
./Peerster -name=tl -peers="$tmAddr,$blAddr" -UIPort="$tlUIPort" -gossipAddr="$tlAddr" -rtimer=10000 -antiEntropy=0 > "./tests/out/TL.out" &
./Peerster -name=tr -peers="$tmAddr,$brAddr" -UIPort="$trUIPort" -gossipAddr="$trAddr" -rtimer=10000 -antiEntropy=0 > "./tests/out/TR.out" &
./Peerster -name=bl -peers="$tlAddr,$brAddr" -UIPort="$blUIPort" -gossipAddr="$blAddr" -rtimer=10000 -antiEntropy=0 > "./tests/out/BL.out" &
./Peerster -name=br -peers="$blAddr,$trAddr" -UIPort="$brUIPort" -gossipAddr="$brAddr" -rtimer=10000 -antiEntropy=0 > "./tests/out/BR.out" &
./Peerster -name=tm -peers="$tlAddr,$trAddr" -UIPort="$tmUIPort" -gossipAddr="$tmAddr" -rtimer=10000 -antiEntropy=0 > "./tests/out/TM.out" &
...
fileHash=$(cat tests/out/TL.out | grep "File $sharedFileName.*" | awk '{print $4}')
...
echo "Kill all the peerster processes..."
sleep 2
pkill -f Peerster  #$(ps aux | grep '\.\/[P]eerster' | awk '{print $2}')
sleep 1
echo "Killed"
