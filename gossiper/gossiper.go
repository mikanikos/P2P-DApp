...
func NewGossiper(name string, address string, peersList []string, uiPort string, simple bool, antiEntropyTimeout int) *Gossiper {
	...
	return &Gossiper{
		...
		originPackets:      PacketsStorage{OriginPacketsMap: originPackets, Messages: make([]RumorMessage, 0)}, //MutexOriginPackets{OriginPacketsMap: originMessagesMap},
		simpleMode:         simple,
...
func (gossiper *Gossiper) handleConnectionClient(channelClient chan *ExtendedGossipPacket) {
	...
	for extPacket := range channelClient {
		...
		//fmt.Println(extPacket.Packet.Simple.OriginalName)
		...
		if gossiper.simpleMode {
			...
			// if !check {
			// 	gossiper.originPackets.OriginPacketsMap[gossiper.name] = make(map[uint32]*ExtendedGossipPacket)
			// }
			// gossiper.originPackets.OriginPacketsMap[gossiper.name][id] = newPacket
...
func (gossiper *Gossiper) handleConnectionRumor(rumorChannel chan *ExtendedGossipPacket) {
	for extPacket := range rumorChannel {
		...
		// } else {
		// 	gossiper.originPackets.OriginPacketsMap[extPacket.Packet.Rumor.Origin] = make(map[uint32]*ExtendedGossipPacket)
		// }
		// if !isMessageKnown {
